    ...
    function forgot_password() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (!empty($user)) {
                ...
                $userData = $this->User_model->getUserById($user['id']);

                $this->load->library('email');
                $this->email->from($this->general->getSetting('Admin.Email'), 'Admin Tokokita');
                $this->email->to($email);

                $this->email->subject('Step Reset Password - Masmet');
                $message = '';
     ...
     function reset_password($email = null, $token = null) {
        ...
        $this->form_validation->set_error_delimiters('','<br/>');
        if ($this->form_validation->run() == TRUE) {
            $this->User_model->resetPassword($resetSession['id'], $this->input->post('password'));
            $this->User_model->truncateToken($resetSession['id']);
